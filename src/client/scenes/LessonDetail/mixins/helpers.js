...
export function calculateResult(result = {}) {
  ...
  // 错误字符排序
  const errorArray = objectToArray(errorChars).sort((a, b) => (a.value - b.value)) || [];
  // wpm = 总字符数 / 时间(s) * 60
  const wpm = Math.round((totalCount / time) * 60) || 0;
  // 输入字符数
  ...
  // 返回结果
  return { totalCount, inputCount, errorCount, wpm, deleteCount, errorArray };
}
